...
export interface ResearchArticleDocument extends Document<Types.ObjectId> {
  ...
  url: string;
  source: string;
  category: 'glp_1' | 'muscle_retention' | 'nutrition' | 'side_effects' | 'peptides';
  publishedAt?: string;
  createdAt: Date;
  updatedAt: Date;
}
...
const researchArticleSchema = new Schema<ResearchArticleDocument>(
  {
    ...
    category: {
      type: String,
      enum: ['glp_1', 'muscle_retention', 'nutrition', 'side_effects', 'peptides'],
      required: true,
      index: true,
    },
    publishedAt: {
...
processedWebhookEventSchema.index({ processedAt: 1 }, { expireAfterSeconds: 60 * 60 * 24 * 90 });
...
export const ProcessedWebhookEventModel = mongoose.model<ProcessedWebhookEventDocument>(
