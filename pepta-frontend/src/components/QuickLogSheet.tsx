...
const OFFSCREEN = 600;
const SIDE_EFFECTS: SideEffectType[] = [
  "nausea",
  "constipation",
  "fatigue",
  "headache",
...
const QUICK_LOG_SHEET_MAX_RATIO = 0.84;

export type QuickLogMode =
  | "chooser"
  | "dose"
  | "weight"
  | "protein"
...
export function QuickLogSheet({
  ...
  const onSave = () => {
    ...
    if (mode === "dose" && isDoseValid(dose)) {
      ...
      const input = toSideEffectInput(
        seTypes,
        severity,
        ts,
        seNote.trim() || undefined,
      );
      commit(
      ...
      const input = toMeasurementInput(
        measureType,
        measureValue,
        measureUnit,
        ts,
        measureNote.trim() || undefined,
      );
      commit(
      ...
      const draft = {
        ...
        workoutMinutes: Number(activity.workoutMinutes) || 0,
        resistanceTraining: activity.resistance,
      };
            ...
            >
              ...
              >
                {mode === "chooser" ? (
                  <Chooser
                    theme={theme}
                    dose={defaultDoseDraft(home, track)}
                    latestWeight={home?.latestWeight ?? null}
                    onPick={openMode}
                    onMeal={onMeal}
                  />
                ) : null}
                ...
                {mode === "measurement" ? (
                  <MeasurementForm
                    ...
                    onStep={(delta) =>
                      setMeasureValue((v) =>
                        Math.max(0, Math.round((v + delta) * 10) / 10),
                      )
                    }
                    unit={measureUnit}
                    onUnit={(next) => {
                      ...
                      setMeasureValue(
                        (v) =>
                          Math.round(
                            (next === "cm" ? inchesToCm(v) : cmToInches(v)) *
                              10,
                          ) / 10,
                      );
                      setMeasureUnit(next);
...
function formatDoseLabel(dose: DoseDraft): string {
  return `${dose.compoundName} · ${dose.amount} ${dose.unit}`;
}

function QuickShotExplainer({
  theme,
  dose,
}: {
  theme: Theme;
...
const HEADINGS: Record<Mode, { title: string; sub: string }> = {
  chooser: {
    title: "Log something",
    sub: "Smart defaults — most logs are one tap.",
  },
  ...
  sideEffect: {
    ...
    sub: "What you’re feeling, and how strong.",
  },
  measurement: {
    title: "Log measurement",
    sub: "Track inches alongside the scale.",
  },
  activity: {
    title: "Log activity",
    sub: "Steps, a workout, or resistance training.",
  },
};
...
function Chooser({
  ...
  const cards: {
    ...
    {
      key: "weight",
      icon: <Icon name="scale" size={22} color={theme.colors.weight} />,
      label: "Log weight",
      hint: latestWeight
        ? `${latestWeight.value} ${latestWeight.unit} ±`
        : "Today’s weigh-in",
      action: () => onPick("weight"),
    },
    {
      key: "meal",
      icon: <Icon name="nutrition" size={22} color={theme.colors.protein} />,
      label: "Log meal",
      hint: "Scan · Search · Voice",
      action: onMeal,
    },
    {
      key: "water",
      icon: <Icon name="water" size={22} color={theme.colors.water} />,
      label: "Water",
      hint: "+8 oz",
      action: () => onPick("water"),
    },
    {
      key: "activity",
      icon: <Icon name="dumbbell" size={22} color={theme.colors.fiber} />,
      label: "Activity",
      hint: "Steps · workout",
      action: () => onPick("activity"),
    },
    {
      key: "sideEffect",
      icon: <Icon name="sad-outline" size={22} color={theme.colors.warning} />,
      label: "Side effects",
      hint: "How you feel",
      action: () => onPick("sideEffect"),
    },
    {
      key: "protein",
      icon: (
        <Icon name="food-drumstick" size={22} color={theme.colors.protein} />
      ),
      label: "Protein",
      hint: "Add grams",
      action: () => onPick("protein"),
    },
    {
      key: "measurement",
      icon: <Icon name="resize" size={22} color={theme.colors.weight} />,
      label: "Measurement",
      hint: "Waist, hips…",
      action: () => onPick("measurement"),
    },
  ];
  return (
    <View
...
function DoseForm({
        ...
        {!loading ? (
          <View style={{ width: 220, marginTop: 6 }}>
            <Button
              ...
              leading={
                <Icon name="needle" size={17} color={theme.colors.onPrimary} />
              }
  ...
  const step = (delta: number) => () => {
    ...
    setDose({
      ...dose,
      amount: Math.max(0, Math.round((dose.amount + delta) * 100) / 100),
    });
  };
...
function WeightForm({
  ...
  const pct = Math.max(
    0,
    Math.min(1, (value - progressMin) / (progressMax - progressMin)),
  );
  return (
    <View style={{ marginTop: 18, alignItems: "center", gap: 16 }}>
      ...
      >
        ...
        <AppText
          variant="cardTitle"
          color="textSecondary"
          style={{ lineHeight: 28, paddingBottom: 8 }}
        >
          {unit}
        </AppText>
      </View>
      <View style={{ width: "82%", gap: 6 }}>
        <ProgressBar
          ...
          trackColor="rgba(226,92,196,0.11)"
          height={5}
        />
  ...
  return (
    <View style={{ marginTop: 18, alignItems: "center" }}>
      ...
      >
        <AppText
          variant="statBig"
          style={{ color, fontSize: 40, lineHeight: 48 }}
        >
          +{value}
        </AppText>
        <AppText
          variant="cardTitle"
          color="textSecondary"
          style={{ lineHeight: 28, paddingBottom: 6 }}
        >
          {unit}
        </AppText>
      </View>
...
function SideEffectForm({
  ...
  theme: Theme;
  selected: SideEffectType[];
  onToggle: (t: SideEffectType) => void;
  severity: number;
          ...
          return (
            <Pressable
              ...
              style={{
                ...
                alignItems: "center",
                justifyContent: "center",
                backgroundColor: active ? tint : theme.colors.surfaceAlt,
              }}
            >
              <AppText
                variant="bodyStrong"
...
function ActivityForm({
  ...
  return (
    <View style={{ marginTop: 16, gap: 11 }}>
      {row(
        "Steps",
        <Icon name="walk" size={18} color={theme.colors.fiber} />,
        activity.steps,
        (v) => setActivity({ ...activity, steps: v }),
        "8000",
      )}
      {row(
        "Workout min",
        <Icon name="time-outline" size={18} color={theme.colors.water} />,
        activity.workoutMinutes,
        (v) => setActivity({ ...activity, workoutMinutes: v }),
        "30",
      )}
      <Pressable
        onPress={() => {
      ...
      >
        ...
        <View
          style={{
            ...
            height: 28,
            borderRadius: 14,
            backgroundColor: activity.resistance
              ? theme.colors.fiber
              : theme.colors.border,
            padding: 3,
            alignItems: activity.resistance ? "flex-end" : "flex-start",
          }}
        >
          <View
            style={{
